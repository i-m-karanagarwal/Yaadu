...
export default function AddBillInput({ onParsed }: Props) {
  ...
  async function parseText(trimmed: string) {
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <label htmlFor="bill-input" className="block text-sm font-medium text-stone-800">
            Type or speak a bill
          </label>
          <p className="mt-1 text-xs text-stone-500">
            Hindi, English, or Hinglish — tap the mic and say it out loud.
          </p>
        </div>
        {speech.supported ? (
      ...
      {speech.listening ? (
        <p className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-medium text-red-700">
          Listening… speak your bill, then tap the mic again to stop. It will parse automatically.
        </p>
      ) : null}
      ...
      <textarea
        ...
        placeholder="Bijli ka bill har mahine 10 tareekh ko, ₹2000 ke aas paas…"
      ...
      {!speech.supported ? (
        <p className="mt-2 text-xs text-stone-500">
          Voice works best in Chrome (desktop or Android). You can still type here.
        </p>
      ) : null}
